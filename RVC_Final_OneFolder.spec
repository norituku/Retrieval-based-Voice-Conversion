...
# librosaモジュールをデータとして含める（PYZアーカイブを回避）
librosa_data = []
try:
    import librosa
    librosa_path = Path(librosa.__file__).parent
    # librosaディレクトリ全体をデータとして追加
    librosa_data.append((str(librosa_path), 'librosa'))
    print(f"Adding librosa as data from: {librosa_path}")
except Exception as e:
    print(f"Warning: Failed to add librosa data: {e}")

# すべてのバイナリとデータを結合
all_binaries = tkinter_binaries + pytorch_binaries + faiss_binaries + pyav_binaries
all_datas = datas + librosa_data
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        'numpy', 'scipy', 'torch', 'torchaudio', 'soundfile',
        # PyTorch C拡張モジュール
        'torch._C',
        'torch._C._dynamo',
        'torch._C._profiler',
    ...
    hooksconfig={},
    runtime_hooks=[],
    excludes=['librosa'],  # librosaはデータとして含めるので除外
    win_no_prefer_redirects=False,
    win_private_assemblies=False,
    cipher=None,
    noarchive=False,  # OneFolderモードでは通常のアーカイブを使用
)

# PYZアーカイブ作成（圧縮レベルを低めに設定）
pyz = PYZ(a.pure, compress_level=3)  # 軽い圧縮を使用
