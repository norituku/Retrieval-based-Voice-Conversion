...
class DarkModeGUIComplete:
    ...
    def load_models_fallback(self):
        ...
        try:
            ...
            if model_files:
                self.display_models_fallback(model_files)
                self.log_message(f"Standard models loaded: {len(model_files)}", "INFO")
            else:
                self.log_message("No models found in model directory", "WARNING")
        except Exception as e:
            self.log_message(f"Fallback model loading error: {e}", "ERROR")
    ...
    def _run_conversion_internal(self):
        ...
        try:
            ...
            output_filename = f"{input_filename}_{model_name}_converted.wav"
            output_path = os.path.join(output_dir, output_filename)
            
            self.log_message(f"Starting conversion: {input_filename}", "INFO")
            ...
            self.log_message(f"❌ Conversion failed: {e}", "ERROR")
